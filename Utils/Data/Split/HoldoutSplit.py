...
def holdout_split_train_test(input_dataset_id: str, pc_hold_out: float = 0.20, timestamp_threshold = 1581465600, random_seed = 888):
    input_file_name = f"{RootPath.get_dataset_path()}/{input_dataset_id}.csv.gz"
    train_file_name = f"{RootPath.get_dataset_path()}/holdout/train.csv.gz"
    val_file_name = f"{RootPath.get_dataset_path()}/holdout/test.csv.gz"
