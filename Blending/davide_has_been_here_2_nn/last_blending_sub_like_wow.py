...
def prediction(LGBM, dataset_id, df, label):
    ...
    create_submission_file(tweets, users, predictions, f"{dataset_id}_{label}_lgbm_blending_submission_wow.csv")
...
def main():
    ...
    # Load val data
    df_val, df_val_label = Data.get_dataset_xgb(val_dataset, real_features, label)

    # Load test data
    df_test = Data.get_dataset(real_features, test_dataset)
    df_private = Data.get_dataset(real_features, private_test_dataset)
    ...
    df_fake_val = pd.DataFrame(columns=features)
    ...
    params = {
        'num_leaves': 1882.5496562720582,
        'learning_rate': 0.03449295602628846,
        'max_depth': 70,
        'lambda_l1': 1.0,
        'lambda_l2': 1.2038178527513668,
        'colsample_bynode': 0.6697273486567108,
        'colsample_bytree': 1.0,
        'bagging_fraction': 0.6080573084633714,
        'bagging_freq': 9,
        'max_bin': 4999.999999999999,
        'min_data_in_leaf': 91.83769775818897,
    }
