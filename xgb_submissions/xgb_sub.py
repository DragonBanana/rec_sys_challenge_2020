...
xgboost = XGBoost(
    ...
    max_depth=15,
    min_child_weight = 56,
    colsample_bytree = 0.6097338094858208,
    learning_rate = 0.09543915250602547,
    reg_alpha = 0.0007409375680410133,
    reg_lambda = 0.07791189651860214,
    scale_pos_weight = 1,
    gamma = 5.612143043060599,
    subsample = 0.74442656286558,
    base_score = 0.4392,
    max_delta_step = 43.822134830500595,
    num_parallel_tree = 3,
)
...
def run_xgb():
    ...
    X_local_val = get_dataset_batch(X_label, val_dataset_id, 2, 0, 0.5)
    Y_local_val = get_dataset_batch(Y_label, val_dataset_id, 2, 0, 0.5)
    ...
    X_remote_val = get_dataset_batch(X_label, val_dataset_id, 2, 1, 0.5)
    Y_remote_val = get_dataset_batch(Y_label, val_dataset_id, 2, 1, 0.5)
