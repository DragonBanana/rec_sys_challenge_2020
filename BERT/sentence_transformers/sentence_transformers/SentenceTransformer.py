...
class SentenceTransformer(nn.Sequential):
    ...
    def encode(self, sentences: List[str], already_tokenized : bool = False, already_padded : bool = False, batch_size: int = 8, show_progress_bar: bool = True, output_value: str = 'sentence_embedding', convert_to_numpy: bool = True) -> List[ndarray]:
        """
        ...
        if show_progress_bar:
            iterator = tqdm(iterator, desc="\tProcessing embeddings batches")
        ...
        for batch_idx in iterator:
            ...
            # if not already padded, pad all the sequences to the same length
            if not already_padded:
                longest_seq = 0
                length_sorted_idx = np.argsort([len(sen) for sen in sentences])
                for idx in length_sorted_idx[batch_start: batch_end]:
                    sentence = sentences[idx]
                    # tokenize the sentence if not already done and append to the batch
                    if not already_tokenized:
                        tokens = self.tokenize(sentence)
                        longest_seq = max(longest_seq, len(tokens))
                        batch_tokens.append(tokens)
                    elif already_tokenized:
                        longest_seq = max(longest_seq, len(sentence))
                        batch_tokens.append(sentence)
            # simply create a list of indices and append the sentence to the batch
            elif already_padded:
                longest_seq = len(sentences[0])
                length_sorted_idx = np.arange(len(sentences))
                for sentence in sentences[batch_start: batch_end]:
                    batch_tokens.append(sentence)

            features = {}
            for tokens in batch_tokens:
                sentence_features = self.get_sentence_features(tokens, longest_seq, already_tokenized, already_padded)
